...
/// <summary>
/// Executes a mission scenario (single or porkchop) with file I/O support.
/// </summary>
public static class ScenarioExecutor
{
    /// <summary>
    /// Executes a scenario without writing files to disk, using default JSON options.
    /// </summary>
    public static ExecutionResult Execute(ScenarioInput input)
        => Execute(input, null, ScenarioJson.Options);

    /// <summary>
    /// Executes a scenario without writing any files to disk.
    /// </summary>
    public static ExecutionResult Execute(ScenarioInput input, JsonSerializerOptions jsonOptions)
        => Execute(input, null, jsonOptions);

    /// <summary>
    /// Executes a scenario with an optional input path for resolving relative output paths.
    /// </summary>
        ...
        OrbitalBody destination)
    {
        ...
        if (inputPath is not null && !string.IsNullOrWhiteSpace(request.ResultOutputPath))
        {
        ...
        OrbitalBody destination)
    {
        ...
        if (inputPath is not null && !string.IsNullOrWhiteSpace(request.CsvOutputPath))
        {
